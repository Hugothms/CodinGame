...
def minimax(game, depth, best_action, time_init=time.time(), time_max=0.009):
    if depth == 0 or game.day == 25 or time.time() - time_init > time_max:
        # print_debug(time.time() - time_init)
        return (game.evalutation_score_position(), best_action)
    max_eval = -math.inf
    for action in game.find_interesting_actions(True):
        ...
        res = minimax(next_game, depth - 1, best_action, time_init, time_max)
        eval = res[0]
        if eval > max_eval:
            print_debug(action)
            max_eval = eval
...
while True:
    ...
    print(minimax(game, 10, None, time.time(), time_max=100)[1])